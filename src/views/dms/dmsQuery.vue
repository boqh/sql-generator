<template>
  <div class="sql-executor-container">
    ...
    <div class="sql-section">
      ...
      <a-textarea id="sqlInput" v-model:value="trimmedSqlQuery" rows="5" />
    </div>
...
// Computed property for trimmed sqlQuery
const trimmedSqlQuery = computed({
  get: () => rawSqlQuery.value,
  set: (val) => {
    rawSqlQuery.value = val ? val.replace(/\s+/g, ' ').trim() : '';
  }
});

// Computed properties for trimmed search params
const trimmedAppId = computed({
  get: () => rawSearchParams.value.appId,
  set: (val) => {
    rawSearchParams.value.appId = val ? val.replace(/\s+/g, '').trim() : '';
  }
});
...
const trimmedMerchantName = computed({
  ...
  set: (val) => {
    rawSearchParams.value.merchantName = val ? val.replace(/\s+/g, ' ').trim() : '';
  }
});
...
const trimmedMerchantNo = computed({
  ...
  set: (val) => {
    rawSearchParams.value.merchantNo = val ? val.replace(/\s+/g, '').trim() : '';
  }
});
...
const trimmedEmail = computed({
  ...
  set: (val) => {
    rawSearchParams.value.email = val ? val.replace(/\s+/g, ' ').trim() : '';
  }
});
...
const trimmedCardNo = computed({
  ...
  set: (val) => {
    rawSearchParams.value.cardNo = val ? val.replace(/\s+/g, '').trim() : '';
  }
});
